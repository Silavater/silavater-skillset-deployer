...
Use `scripts\Deploy-SkillSet-Py.cmd` on Windows for the Python deployer. It supports interactive selection, explicit `--target`, curated sets, individual skills, and dry runs.
...
.\scripts\Deploy-SkillSet-Py.cmd --skill matt:handoff --skill ecc:terminal-ops
```
