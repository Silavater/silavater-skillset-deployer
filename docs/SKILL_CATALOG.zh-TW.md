...
由 `scripts/Deploy-SkillSet.cmd -UpdateCatalog -DryRun` 從本機鏡像產生。

> 說明：本繁體中文版本會翻譯導覽、建議技能集、部署指令與注意事項；完整技能清單保留技能名稱、觸發語意與本機路徑，以避免改寫上游技能描述造成觸發條件失真。
...
此目錄引用下列第三方 skill repositories 的本機鏡像，以支援離線且可重複的部署：
...
`vendor/skill-sources/` 內的鏡像 skill 來源於上述上游專案。本 repository 是獨立且非官方的打包／部署輔助工具；除非另有明確說明，否則並非上游作者官方維護、認可或發行。

使用、修改或重新散布這些鏡像來源時，請保留上游 license files 與 attribution notices。原始來源、文件、歷史紀錄與授權條款請參考 `../THIRD_PARTY_NOTICES.md` 與上游 repositories。

## 建議技能集

> 建議：一次只部署一到兩個技能集。除非你明確想要一個很吵、很大的代理技能操作介面，否則不要主動安裝所有 ECC 技能。完整來源已鏡像到 `vendor/skill-sources/`，需要時再部署即可。

### core-dev

日常開發：程式碼理解、除錯、TDD、交接與驗證。
...
TypeScript 後端：API、資料庫、ORM、快取、Node／Next／Nest 模式。
...
Python：慣用寫法、pytest、Django、資料與機器學習工作流程。
...
代理操作：技能偵察、盤點、品質、上下文、平行化與本機知識管理。
...
Windows 上請優先使用 `scripts\Deploy-SkillSet.cmd`。它會用單次程序的 execution-policy bypass 啟動 PowerShell 實作，可避開未簽署腳本造成的 `PSSecurityException`，而不需要修改系統原則。
...
CC Switch 可以另外評估。它更接近環境／模型／設定切換器，而不是此技能目錄的替代品。建議先完成本機技能鏡像與部署腳本，再把 CC Switch 加入 `env-setup` 工作流程，或另開一個環境切換任務處理。

## 已知問題：`reasoning part 0 not found`

這通常是 session viewer／gateway／模型 fallback 的中繼資料解析問題：呼叫端嘗試讀取 `reasoning[0]`，但模型回應沒有該區段。如果 CLI 指令已完成且檔案有產出，通常不會阻擋技能鏡像或部署。除錯時可檢查模型名稱、fallback provider、OpenCode session viewer，以及 gateway 是否支援 reasoning parts。

## 完整本機技能目錄

| 儲存庫 | 技能 | 用途（保留上游原文） | 本機檔案 |
|---|---|---|---|
